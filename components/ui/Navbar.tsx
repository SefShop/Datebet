...
export default function Navbar({ active = 'home', onActivityTap, unread = 0 }: NavbarProps) {
  ...
  return (
    <div className="flex justify-around px-3 pt-2.5 pb-7 flex-shrink-0"
      style={{ borderTop: '1px solid rgba(255,255,255,0.06)', background: 'rgba(10,10,16,0.95)' }}>
      {items.map(item => {
        ...
        return (
          <div key={item.id}
            ...
            style={{ color: isOn ? '#fd297b' : 'rgba(255,255,255,0.3)' }}>
            <span className="text-[22px] leading-none relative">
              ...
              {item.badge && item.badge > 0 && (
                <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 rounded-full
                  ...
                  style={{ background: 'linear-gradient(135deg, #fd297b, #ff655b)', boxShadow: '0 1px 6px rgba(253,41,123,0.6)' }}>
                  {item.badge}
                </span>
              )}
            </span>
