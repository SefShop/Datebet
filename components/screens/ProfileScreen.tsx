...
export default function ProfileScreen() {
  ...
  const p = hasProfiles ? profiles[idx % profiles.length] : null
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ background:'#06060a' }}>
      ...
      <div className="flex-1 px-3 pb-2 flex items-center justify-center">
        {loading ? (
          <div className="text-center">
            <div className="text-[32px] mb-3" style={{ animation:'pulse 1s infinite' }}>🎮</div>
            <div className="text-[13px] text-white/40">{lang==='gr'?'Φόρτωση παικτών...':'Loading players...'}</div>
          </div>
        ) : !hasProfiles || !p ? (
          ...
          <div key={p.id + idx} className="w-full max-w-[400px] rounded-3xl overflow-hidden relative"
            ...
            <div className="relative h-[340px] overflow-hidden" style={{ background: p.gradient }}>
              {p.photo ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-[80px]" style={{ background: p.gradient }}>
                  <span style={{ filter: 'drop-shadow(0 4px 16px rgba(0,0,0,0.3))' }}>👤</span>
                </div>
              )}
      ...
      {/* Action buttons (hide if no profiles) */}
      {hasProfiles && !loading && (
