...
export default function GameSelectionScreen() {
  ...
  const games = [
    { key:'c4',  emoji:'🔴', title:t.c4Title,  desc:t.c4Desc,  label:t.c4Label,  glow:'#fd297b', go:()=>navigate('connect4')  },
    { key:'ttt', emoji:'❌', title:t.tttTitle, desc:t.tttDesc, label:t.tttLabel, glow:'#38bdf8', go:()=>navigate('tictactoe') },
    { key:'ludo',emoji:'🎲', title:t.ludoTitle,desc:t.ludoDesc,label:t.ludoLabel,glow:'#a78bfa', go:()=>navigate('ludo')      },
  ]
  ...
  return (
    <div className="flex flex-col h-full px-6 pt-16 pb-8"
      ...
      <p className="text-[14px] mb-7" style={{ color:'rgba(255,255,255,0.4)' }}>{t.selectSub}</p>
      ...
      <div className="flex flex-col gap-3.5 flex-1">
        {games.map((g,i) => (
          <button key={g.key} onClick={g.go}
            ...
            style={{
              background:'rgba(255,255,255,0.04)', border:`1px solid ${g.glow}30`,
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[13px]" style={{ color:'rgba(255,255,255,0.45)' }}>{g.desc}</div>
              </div>
